...
var DIRECTION_MAPS = {
  ...
  BORDERS: {
    NW: { q: 0, r: 0, neighborBorderKey: 'NW' },
    NE: { q: +1, r: -1, neighborBorderKey: 'SW' },
    E: { q: +1, r: 0, neighborBorderKey: 'W' },
    SE: { q: 0, r: +1, neighborBorderKey: 'NW' },
    SW: { q: 0, r: 0, neighborBorderKey: 'SW' },
    W: { q: 0, r: 0, neighborBorderKey: 'W' }
  }
};
...
Board.prototype.assignCorners = function () {
  this.tiles.forEach(tile => {
    Object.keys(tile.corners).forEach(direction => {
      ...
      if (neighborTile) {
        corner = neighborTile.corners[coordTranslate.neighborCornerKey];
      } else {
        corner = new Corner();
      }
...
Board.prototype.assignBorders = function () {
  this.tiles.forEach(tile => {
    Object.keys(tile.borders).forEach(direction => {
      ...
      if (neighborTile === tile) return;
      if (neighborTile) {
        border = neighborTile.borders[coordTranslate.neighborBorderKey];
      } else {
        border = new Border();
      }
...
Tile.prototype.makeCornerPoints = function () {
  return Object.keys(this.corners).map(function (key) {
    ...
    var i = DIRECTION_MAPS.CORNERS[key].index;
